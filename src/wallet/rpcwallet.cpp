...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"litecoinzaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"litecoinzaddress\"  (string, required) The LitecoinZ address to use for the private key.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid LitecoinZ address: ")+name_);
...
UniValue z_listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "z_listunspent ( minconf maxconf  [\"zaddr\",...] )\n"
            ...
            "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
            "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
            "3. \"addresses\"      (string) A json array of zaddrs to filter\n"
            "    [\n"
            ...
            + HelpExampleCli("z_listunspent", "6 9999999 \"[\\\"ztJnEGuwu1y2bHybMLGsRzCMfF582kGZ4hncdDKePyG7TU8oPYmVBqhebgDDkV66sjbDb4TnmJqhLZQJRQP4XPdu9HfVhyB\\\",\\\"ztrfxBWjuAY5P34ALyCPJjoW5kXvj9gAYoh7Rar9u2sj3xPPt6B7ztGG5ZbXxPrn3t9shiW5bdxgEBfwUePYaxmR7XZ1P7B\\\"]\"")
            + HelpExampleRpc("z_listunspent", "6, 9999999 \"[\\\"ztYvEWFxFs1WnePGB889bZycPdkhqeqtXomvn8yci9a2RFJSSWt2P8yt9d5dZq3KAYPSgRkiwCmcbQdywRZPsrMhpkPePDB\\\",\\\"ztrfxBWjuAY5P34ALyCPJjoW5kXvj9gAYoh7Rar9u2sj3xPPt6B7ztGG5ZbXxPrn3t9shiW5bdxgEBfwUePYaxmR7XZ1P7B\\\"]\"")
        );
    ...
    if (params.size() > 2) {
        UniValue addresses = params[2].get_array();
        if (addresses.size()==0)
        ...
        for (const UniValue& o : addresses.getValues()) {
            ...
            try {
                ...
                if (!pwalletMain->HaveSpendingKey(addr)) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, address cannot be watch only: ") + address);
                }
                zaddrs.insert(addr);
    ...
    if (zaddrs.size() > 0) {
        ...
        pwalletMain->GetUnspentFilteredNotes(entries, zaddrs, nMinDepth, nMaxDepth);
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "\nResult:\n"
            "\"litecoinzaddress\"    (string) The new zaddr\n"
            "\nExamples:\n"
