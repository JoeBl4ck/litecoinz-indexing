...
class ZkeyImportExportTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def find_imported_key(node, import_zaddr):
            ...
            for zdr in zaddrs:
                if zdr == import_zaddr:
                    ...
                    print("Imported zaddr {0} not found".format(import_zaddr))
        ...
        assert_equal(bob_zaddr, ipk_zaddr)
        ...
        assert_equal(bob_zaddr, ipk_zaddr2)
        ...
        # Reproduce zombie balance reported in #1936
        ...
        assert_equal(bob_zaddr, d_ipk_zaddr)
        # Bug: Check if already-spent balance from bob is unspent for david
        # UTXOs should equal amounts[2:], not amounts
        verify_utxos(david, amounts, d_ipk_zaddr)
        # Check if UTXOs bob spent are removed for Charlie
        verify_utxos(charlie, amounts, ipk_zaddr)
        # if so, try to spend, get "nullifier spent" error.
